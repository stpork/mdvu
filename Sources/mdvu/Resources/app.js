...
const renderDiagrams=async()=>{const mermaidNodes=[...document.querySelectorAll('.diagram-pending[data-renderer="mermaid"]')];if(mermaidNodes.length&&typeof mermaid!=='undefined'){if(window['mermaid-zenuml']&&!window.__zenumlRegistered){try{await mermaid.registerExternalDiagrams([window['mermaid-zenuml']]);window.__zenumlRegistered=true}catch(err){console.error('ZenUML registration error:',err)}}const selected=document.documentElement.dataset.theme;const dark=selected==='dark'||selected==='system'&&matchMedia('(prefers-color-scheme:dark)').matches;mermaid.initialize({startOnLoad:false,securityLevel:'strict',theme:dark?'dark':'default',themeVariables:{background:'transparent'},suppressErrorRendering:true});for(let i=0;i<mermaidNodes.length;i++){const node=mermaidNodes[i],source=node.querySelector('pre').textContent,key=node.dataset.cacheKey;try{await new Promise(r=>requestAnimationFrame(r));const result=await mermaid.render(`mdvu-diagram-${i}-${Date.now()}`,source);node.innerHTML=result.svg;node.classList.remove('diagram-pending');if(window.webkit?.messageHandlers?.diagramCache)window.webkit.messageHandlers.diagramCache.postMessage({key,svg:result.svg})}catch(e){node.classList.add('diagram-error');node.innerHTML=`<strong>Diagram could not be rendered</strong><pre></pre><small></small>`;node.querySelector('pre').textContent=source;node.querySelector('small').textContent=String(e.message||e)}}}const pumlNodes=[...document.querySelectorAll('.diagram-pending[data-renderer="plantuml"]')];if(pumlNodes.length&&typeof PlantUML!=='undefined'&&PlantUML.renderToString){const selected=document.documentElement.dataset.theme;const dark=selected==='dark'||selected==='system'&&matchMedia('(prefers-color-scheme:dark)').matches;for(let i=0;i<pumlNodes.length;i++){const node=pumlNodes[i],rawSource=node.querySelector('pre').textContent,key=node.dataset.cacheKey;try{await new Promise(r=>requestAnimationFrame(r));let lines=rawSource.trim().split(/\r?\n/);if(!lines.some(l=>/^\s*@start/i.test(l))){lines.unshift('@startuml');lines.push('@enduml')}if(dark&&!lines.some(l=>/^\s*skinparam\s+backgroundColor/i.test(l))){lines.splice(1,0,'skinparam backgroundColor transparent')}await new Promise((resolve,reject)=>{PlantUML.renderToString(lines,svg=>{if(!svg){reject(new Error('PlantUML returned empty output'));return}node.innerHTML=svg;node.classList.remove('diagram-pending');if(window.webkit?.messageHandlers?.diagramCache)window.webkit.messageHandlers.diagramCache.postMessage({key,svg});resolve()},err=>reject(new Error(err)))})}catch(e){node.classList.add('diagram-error');node.innerHTML=`<strong>Diagram could not be rendered</strong><pre></pre><small></small>`;node.querySelector('pre').textContent=rawSource;node.querySelector('small').textContent=String(e.message||e)}}}};
